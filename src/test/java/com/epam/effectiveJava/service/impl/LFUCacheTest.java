...
class LFUCacheTest {
    ...
    void shouldAddAndRemoveObjects() {
        // crunchifyTimeToLive = 200 seconds
        // crunchifyTimerInterval = 500 seconds
        // maxItems = 6
        LFUCache cache = new LFUCache(200, 500, 6);
    ...
    void shouldExpiredCacheObjects() {
        // crunchifyTimeToLive = 5 second
        // crunchifyTimerInterval = 5 second
        // maxItems = 10
        LFUCache cache = new LFUCache(5, 5, 10);
        cache.put(1, new ValueObject("eBay"));
        cache.put(2, new ValueObject("Paypal"));
        ...
        LOG.info(String.format("Two objects are added but reached timeToLive. cache.size(): %s", cache.size()));
